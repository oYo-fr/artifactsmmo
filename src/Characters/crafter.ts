 ...
 * Represents a farmer
 */
export class Crafter extends Character {
  /**
   * Constructor
  ...
  protected chooseAction<T extends Action>(actionsCraft: T[], actionsFight: T[], actionsFarm: T[], actionsCook: T[]): Promise<T | undefined> {
    const sortedCraftActions = _.orderBy(actionsCraft, [(action: any) => Bank.getItemQuantity(action.Element.code)], ['asc']);
    const actions = _.concat(sortedCraftActions, actionsFight, actionsFarm, actionsCook);
